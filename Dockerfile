...
RUN <<EOF
    ...
    if [ "$upx" = "1" ] && [ "$profile" != "dev" ]; then
        upx --best target/x86_64-unknown-linux-musl/$profile/terracotta
    fi
EOF
...
WORKDIR /usr/src
COPY --from=builder /usr/src/terracotta/target/x86_64-unknown-linux-musl/$profile/terracotta ./
COPY Config.toml ./
RUN mkdir content html static

EXPOSE 8000

CMD ["/usr/src/terracotta"]
